...
For testing purpose, admin-on-rest provides a `jsonRestClient` function that simply takes a JavaScript object, and serves as a regular REST client.

```js
// in src/App.js
import React from 'react';
import { jsonRestClient, Admin, Resource } from 'admin-on-rest';
